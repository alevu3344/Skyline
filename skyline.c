...
int main( int argc, char* argv[] )
{
    ...
    read_input(&points);
    char filename[32];
        sprintf(filename, "RANGO0");
        FILE *fp = fopen(filename, "w");
        if (!fp)
        {
            fprintf(stderr, "Rank led to open file for writing\n");
    
        }


        // Write the dimension and number of skyline points
        fprintf(fp, "%d\n", points.D);
        fprintf(fp, "%d\n", points.N);

        // Write each local_point to the file
        for (int i = 0; i < points.N; i++)
        {
            for (int k = 0; k < points.D; k++)
            {
                fprintf(fp, "%f ", points.P[i * points.D + k]);
            }
            fprintf(fp, "\n");
        }
